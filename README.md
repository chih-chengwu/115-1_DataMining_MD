# 📚 資料探勘 (Data Mining)

> 慈濟大學 資訊科技與管理學系
> Data Mining Course Materials

---

### 📌 [課堂規定, 課程設計及成績評量PPT](第一週_1.課堂規定_成績評量標準及課程設計.pptx)

---

# 📂 課程目錄

| 章節 | 內容                                                      |
| ---- | --------------------------------------------------------- |
| 01   | [資料探勘 (Data Mining)-01.md : 什麼是資料探勘？](./01.md) |
| 02   | [資料探勘 (Data Mining)-02](./02.md)                       |
| 03   | [資料探勘 (Data Mining)-03](./03.md)                       |
| 04   | [資料探勘 (Data Mining)-04](./04.md)                       |
| 05   | [資料探勘 (Data Mining)-05](./05.md)                       |
| 06   | [資料探勘 (Data Mining)-06](./06.md)                       |
| 07   | [資料探勘 (Data Mining)-07](./07.md)                       |
| 08   | [資料探勘 (Data Mining)-08](./08.md)                       |
| 09   | [資料探勘 (Data Mining)-09](./09.md)                       |
| 10   | [資料探勘 (Data Mining)-10](./10.md)                       |

---

## 📌 使用方式

1. 點選左側課程章節
2. GitHub 會在右側直接顯示教材內容
3. 不需下載即可閱讀
4. 可直接觀看 Markdown、圖片、程式碼及 Notebook

---

## 👨‍🏫 授課教師

**吳志成 教授**

Department of Information Technology and Management

Tzu Chi University

---

## ⭐ 更新紀錄

| 日期 | 說明                    |
| ---- | ----------------------- |
| 2026 | 建立課程教材 Repository |

---

# Happy Learning! 🎉
